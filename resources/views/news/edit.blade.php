...
<x-form :action="route('newsfeed.update', [tenant('tenant_id'), $newsfeed->id])" put id="upload-form">
    <div class="row">
        <x-input.text required
            name="title"
            container-class="col-xs-12 col-sm-12 col-md-12 col-lg-12" value="{!! $newsfeed->title !!}"
        />
        <x-input.textarea
        ...
        <x-select required name="featured_image_placement" container-class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
            @foreach (\App\Models\Newsfeeds::imagePlacementOption() as $value => $label)
                <option value="{{ $value }}" @if ($newsfeed->image_placement == $value) selected @endif>{{ $label }}</option>
            @endforeach
        </x-select>
        <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 form-group">
            ...
            <input type="number" name="excerpt_length" class="form-control" default="250" value="{{$newsfeed->excerpt_length}}" required>
        </div>
        <input type="hidden" name="old_image_url" id="old_image_url" value="{{$newsfeed->image}}">
    </div>
    @if (in_array('NEWS_FEEDS_ALL_CONTENT', $userPerms) || user()->account_type == \App\Models\User::INTERNAL_TENANT_ADMIN)
            ...
            <div class="col-12 ">
                <div class="custom-control custom-radio">
                    <input type="radio" id="post_all_tenants" value="post_all_tenants"
                        ...
                        class="custom-control-input" required @if ($newsfeed->selected == 'post_all_tenants') checked @endif>
            ...
            <div class="col-12">
                <div class="custom-control custom-radio">
                    <input type="radio" id="post_selected_tenants" value="post_selected_tenants"
                        name="post_tenants_type"
                        class="custom-control-input" required @if ($newsfeed->selected == 'post_selected_tenants') checked @endif>
                    <label class="custom-control-label"
        ...
        <input type="hidden" name="tenants" value={{$newsfeed->tenants}}>
    @endif
    <div class="row text-right">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 pt-4">

            <x-button type="button" sm pill id="btn-update-newsfeed" >{{ __('Save') }}</x-button>
...
<script type="text/javascript">
    ...
    var selected_tenants = @json($newsfeed->tenants);
    ...
    $(document).on('change', 'input[name="post_tenants_type"]', function () {
        ...
        if ($(this).val() == "post_selected_tenants") {
            return populateTenantsNewsfeeds(tenants, selected_tenants );
        }
    ...
    $(document).ready(function () {
        ...
        if (tenant_post_setting == "post_selected_tenants") {
            populateTenantsNewsfeeds(tenants, selected_tenants);
        }else{
            $('#tenant_news_div').empty().hide();
        }
    ...
    $(document).on('click', '#btn-update-newsfeed', function(e) {
    ...
    @if(!empty($newsfeed->image) && file_exists(storage_path('app/public/' . $newsfeed->image)))

        var mockFile = {name: "{{$newsfeed->image_name}}", size: {{\File::size(storage_path('app/public/'.$newsfeed->image))}}};
        ...
        //Get the file Extension
        var fileExtension = ("{{$newsfeed->image_name}}").match(patternFileExtension);
       ...
       var image_file = `{{ asset('/storage/' . $newsfeed->image) }}`;

        var icon_file = `{{asset(\Storage::url("icons/files"))}}/${default_icon_image}`;
        $('.dz-message').find('.dz-default-message').addClass('d-none');

        dropzone.emit("addedfile", mockFile);
        dropzone.emit("thumbnail", mockFile, image_file);
