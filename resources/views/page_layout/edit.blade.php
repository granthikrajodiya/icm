...
<div id="data-content" style="display: none">
    <x-form :action="route('layout.update', [tenant('tenant_id'), $layout->id])" put id="form_layout">
        <div class="row">
            <x-select
                required
                name="content_type"
            ...
            <x-input.text
                ...
                :value="$layout->eform_url"
                container-class="d-none eform_div"
                :maxlength="100"
            />
            <input type="hidden" name="adv_config" id="adv_config" value="{{$color_table}}">
            <div class="col-6 mt-3 mb-5 custom-control custom-switch" id="list_mode_toggle">
                <input
                    ...
                    id="list_mode"
                    name="list_mode"
                    {{ $list_mode_settings->list_mode == "on" ? "checked" : "" }}
                >
            ...
            <div x-show="!hideMaxItems" class="col-6" id="max_column_input">
                <x-input.text
                    ...
                    max="5"
                    min="1"
                    :value="$list_mode_settings->max_column"
                />
            </div>
        </div>
        <div class="pt-3">
            <x-button sm pill right id="submit_layout">{{ __('Update Card') }}</x-button>
...
<script>
    ...
    // if content type is changed but not content/workflow view replace the value of list mode and max column to 0
    $("#content_type").change(function() {
        $("#max_column").val(0);
        $("#list_mode").removeAttr("checked");
        // console.log($("#list_mode").val())
    });
